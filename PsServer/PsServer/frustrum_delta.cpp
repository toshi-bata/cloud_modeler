...
PK_ERROR_code_t FRU_delta_write( PK_DELTA_t key, unsigned n_bytes, const char *bytes)
{
    size_t n_copy;
    delta_p_t delta;
    unsigned int offset;  // Changed from int to avoid compiler warnings
	...
	/// The cast in the following line is safe because n_copy is either
	/// less than block_size (which is fixed at 1024) or less than n_bytes,
	/// where n_bytes in practice has a size limited to be significantly 
	/// smaller than the maximum value representable by 'int'
    for ( ; n_bytes > offset; offset += (int)n_copy )
    {
     ...
     memcpy_s( (void *) (delta->current_block->data + delta->current_block->
     used), (block_size - delta->current_block->used), (void *) (bytes + offset), (size_t) n_copy );
...
PK_ERROR_code_t FRU_delta_read( PK_DELTA_t key, unsigned n_bytes, char *bytes)
{
    ...
    for ( ; n_bytes > offset; offset += n_copy )
    {
        ...
        n_copy = ((n_bytes - offset) > (block_size - (unsigned)delta->offset) // Cast to avoid compiler warnings
                     ...
                     : (n_bytes - offset));
        if (bytes != NULL)
            memcpy_s( (void *) (bytes + offset), (block_size - (unsigned)delta->offset), 
			    (void *) (delta->current_block->data + delta->offset), (size_t) n_copy );
        delta->offset += n_copy;
    }
    return PK_ERROR_no_errors;
}
